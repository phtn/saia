...
export const CLIENT_SECTION: FormSection = {
  ...
  fields: [
    { key: 'client_name', label: 'Client name', type: 'text', required: true, placeholder: 'Juan Dela Cruz', aliases: ['name', 'insured name', 'customer'] },
    { key: 'insured_mobile_number', label: 'Mobile number', type: 'tel', required: true, placeholder: '+63 917 000 0000', aliases: ['mobile', 'phone', 'phone number', 'cellphone number'] },
    { key: 'insured_email_address', label: 'Email address', type: 'email', placeholder: 'client@email.com', aliases: ['email'] },
...
export const POLICY_SECTION: FormSection = {
  ...
  fields: [
    { key: 'insurer_name', label: 'Insurer', type: 'select', options: ['Bethel General Insurance', 'Malayan Insurance', 'Pioneer Insurance', 'Standard Insurance', 'FPG Insurance', 'Sun Life', 'AXA Philippines'], aliases: ['carrier', 'insurance company'] },
    { key: 'inception_from', label: 'Inception from', type: 'date', required: true, aliases: ['start date', 'effective date'] },
...
const motorSections: FormSection[] = [
  {
    ...
    title: 'Vehicle',
    fields: [
      { key: 'vehicle_label', label: 'Vehicle', type: 'text', required: true, placeholder: '2024 Toyota Vios 1.3 XLE', aliases: ['car', 'make and model'] },
      { key: 'vehicle_plate', label: 'Plate', type: 'text', placeholder: 'ABC 1234', aliases: ['plate number'] },
      { key: 'vehicle_type', label: 'Vehicle type', type: 'select', options: ['Private car', 'SUV / AUV', 'Pick-up', 'Motorcycle', 'Commercial vehicle'], required: true },
      ...
      { key: 'mortgagee', label: 'Mortgagee', type: 'text', hint: 'Bank or financing company, if the vehicle is financed.' }
    ]
  },
  {
    id: 'coverage',
    title: 'Coverage',
    fields: [
      { key: 'declared_value', label: 'Declared value', type: 'money', required: true, min: 100000, aliases: ['fair market value', 'value', 'sum insured'] },
      { key: 'vtpl_bi', label: 'VTPL bodily injury', type: 'select', options: ['₱100,000', '₱200,000', '₱300,000', '₱500,000'] },
      { key: 'vtpl_pd', label: 'VTPL property damage', type: 'select', options: ['₱100,000', '₱200,000', '₱300,000', '₱500,000'] },
      { key: 'aon', label: 'Acts of Nature', type: 'boolean', hint: 'Flood, typhoon, earthquake.', aliases: ['acts of god', 'flood cover'] },
...
const OD_RATE: Record<string, number> = { 'Private car': 0.013, 'SUV / AUV': 0.0135, 'Pick-up': 0.014, Motorcycle: 0.03, 'Commercial vehicle': 0.018 }
...
const ctplSections: FormSection[] = [
  {
    ...
    title: 'Vehicle',
    fields: [
      { key: 'vehicle_label', label: 'Vehicle', type: 'text', required: true, placeholder: '2019 Honda Click 125i', aliases: ['car', 'make and model'] },
      { key: 'vehicle_plate', label: 'Plate', type: 'text', required: true, aliases: ['plate number'] },
      { key: 'vehicle_type', label: 'Vehicle type', type: 'select', required: true, options: ['Private car', 'Motorcycle / tricycle', 'Light truck', 'Heavy truck', 'Public utility'] },
      { key: 'class_label', label: 'Term', type: 'select', options: ['1 year', '3 years (brand new)'], aliases: ['category', 'category term'] },
      { key: 'mv_file_number', label: 'MV file no', type: 'text', aliases: ['file number'] }
    ]
  }
]

const CTPL_RATES: Record<string, number> = { 'Private car': 560, 'Motorcycle / tricycle': 250, 'Light truck': 610, 'Heavy truck': 1200, 'Public utility': 1100 }
...
const mobileSections: FormSection[] = [
  {
    ...
    title: 'Device',
    fields: [
      { key: 'vehicle_label', label: 'Device', type: 'text', required: true, aliases: ['phone', 'phone model', 'device model'] },
      { key: 'imei', label: 'IMEI', type: 'text', required: true, placeholder: '15 digits', aliases: ['imei number', 'serial'] },
      { key: 'declared_value', label: 'Insurable value (FMV)', type: 'money', required: true, aliases: ['value', 'fair market value', 'price'] },
...
const travelSections: FormSection[] = [
  {
    ...
    title: 'Trip',
    fields: [
      { key: 'insured_person', label: 'Insured person', type: 'text', required: true, aliases: ['traveler', 'traveller'] },
      { key: 'trip_type', label: 'Trip type', type: 'select', options: ['Single trip', 'Annual multi-trip'], required: true },
      { key: 'destination_region', label: 'Destination region', type: 'select', required: true, options: ['Domestic', 'Asia', 'Worldwide excl. USA/Canada', 'Worldwide'], aliases: ['region'] },
      { key: 'destination', label: 'Destination', type: 'text', placeholder: 'Tokyo, Japan', aliases: ['country', 'city'] },
      { key: 'departure_date', label: 'Departure date', type: 'date', required: true, aliases: ['departure', 'leaving'] },
...
const TRAVEL_DAILY: Record<string, number> = { Domestic: 25, Asia: 60, 'Worldwide excl. USA/Canada': 90, Worldwide: 120 }
...
const paSections: FormSection[] = [
  {
    ...
    fields: [
      ...
      { key: 'occupation_class', label: 'Occupation class', type: 'select', required: true, options: ['Class 1 — office / professional', 'Class 2 — light manual', 'Class 3 — heavy manual'], aliases: ['risk class'] },
      { key: 'sum_assured', label: 'Sum assured', type: 'money', required: true, min: 50000, aliases: ['coverage amount', 'sum insured'] },
      { key: 'medical_reimbursement', label: 'Medical reimbursement', type: 'boolean', hint: 'Adds 10% of sum assured for accident medical costs.' },
...
const commercialSections: FormSection[] = [
  {
    ...
    title: 'Business',
    fields: [
      { key: 'business_name', label: 'Business name', type: 'text', required: true, aliases: ['company', 'company name'] },
      { key: 'tin', label: 'TIN', type: 'text', aliases: ['tax number'] },
      { key: 'occupancy', label: 'Occupancy', type: 'select', required: true, options: ['Office', 'Retail / restaurant', 'Warehouse', 'Manufacturing'], aliases: ['business type', 'use'] },
      { key: 'construction_class', label: 'Construction class', type: 'select', required: true, options: ['Class A — concrete', 'Class B — mixed', 'Class C — light materials'], aliases: ['construction'] },
      { key: 'client_address', label: 'Risk address', type: 'text', wide: true, required: true, aliases: ['location', 'property address'] }
    ]
  },
  {
    id: 'sums',
    ...
    fields: [
      ...
      { key: 'business_interruption', label: 'Business interruption', type: 'boolean', hint: 'Loss of gross profit after an insured event.' },
      { key: 'cgl_limit', label: 'General liability limit', type: 'select', options: ['None', '₱1,000,000', '₱3,000,000', '₱5,000,000'], aliases: ['liability'] }
    ]
  }
]

const COMMERCIAL_OCCUPANCY: Record<string, number> = { Office: 0.002, 'Retail / restaurant': 0.003, Warehouse: 0.0045, Manufacturing: 0.006 }
...
const petSections: FormSection[] = [
  {
    ...
    fields: [
      ...
      { key: 'pre_existing', label: 'Pre-existing conditions', type: 'textarea', wide: true, hint: 'Excluded from cover; list them for underwriting.' }
    ]
  }
]
...
const lifeSections: FormSection[] = [
  {
    ...
    fields: [
      { key: 'insured_person', label: 'Insured person', type: 'text', required: true },
      { key: 'insured_birthday', label: 'Birthdate', type: 'date', required: true, aliases: ['birthday', 'date of birth'] },
      { key: 'gender', label: 'Sex', type: 'select', options: ['Female', 'Male'], aliases: ['gender'] },
      { key: 'smoker', label: 'Smoker', type: 'boolean', aliases: ['smokes'] },
      { key: 'sum_assured', label: 'Sum assured', type: 'money', required: true, min: 100000, aliases: ['face amount', 'coverage amount'] },
      { key: 'term_years', label: 'Term', type: 'select', required: true, options: ['5 years', '10 years', '15 years', '20 years'], aliases: ['term'] },
      { key: 'beneficiary', label: 'Beneficiary', type: 'text', wide: true, aliases: ['beneficiaries'] }
    ]
  }
]
...
const homeSections: FormSection[] = [
  {
    ...
    title: 'Property',
    fields: [
      { key: 'vehicle_label', label: 'Property', type: 'text', required: true, placeholder: '2-storey house', aliases: ['property type', 'house'] },
      { key: 'client_address', label: 'Property address', type: 'text', wide: true, required: true, aliases: ['location'] },
...
export const PRODUCTS: Record<ProductId, ProductDefinition> = {
  motor: {
    ...
    defaults: { vehicle_type: 'Private car', fuel_type: 'Gasoline', vtpl_bi: '₱200,000', vtpl_pd: '₱200,000', aon: false, auto_pa: true },
    ...
    rate(values) {
      ...
      const lines: PremiumLine[] = [
        { label: 'Own damage & theft', amount: value * (OD_RATE[str(values, 'vehicle_type')] ?? 0.013), note: 'of declared value' },
  ...
  ctpl: {
    ...
    describeRisk: (values) => [str(values, 'vehicle_label'), str(values, 'vehicle_plate')].filter(Boolean).join(' · ') || 'Vehicle',
    rate(values) {
      ...
      return buildBreakdown([{ label: `CTPL basic premium × ${years} yr`, amount: base * years }], NON_LIFE_TAXES, issues)
    }
  },
  mobile: {
    id: 'mobile',
  ...
  travel: {
    ...
    rate(values) {
      ...
      if (str(values, 'trip_type') === 'Annual multi-trip') {
        return buildBreakdown([{ label: 'Annual multi-trip', amount: daily * 60 * plan * ageFactor, note: 'up to 90 days per trip' }], NON_LIFE_TAXES, issues)
      }
      const days = daysBetween(str(values, 'departure_date'), str(values, 'return_date')) + 1
      if (str(values, 'departure_date') && str(values, 'return_date') && days < 1) issues.push('Return date must be on or after departure.')
      const lines: PremiumLine[] = [{ label: `${Math.max(days, 0)} day(s) × ₱${daily}/day`, amount: Math.max(300, Math.max(days, 0) * daily * plan) }]
  ...
  'personal-accident': {
    ...
    sections: paSections,
    defaults: { occupation_class: 'Class 1 — office / professional', sum_assured: '500000', medical_reimbursement: true },
    ...
    rate(values) {
      ...
      const lines: PremiumLine[] = [{ label: 'Accidental death & disablement', amount: sum * (PA_RATE[cls] ?? PA_RATE[0]) }]
      if (flag(values, 'medical_reimbursement')) lines.push({ label: 'Medical reimbursement', amount: sum * 0.1 * 0.01 })
      return buildBreakdown(lines, NON_LIFE_TAXES, issues)
    }
  },
  commercial: {
    ...
    defaults: { occupancy: 'Office', construction_class: 'Class A — concrete', cgl_limit: 'None', business_interruption: false },
    ...
    rate(values) {
      ...
      const cls = Number(str(values, 'construction_class').match(/Class ([ABC])/)?.[1]?.charCodeAt(0) ?? 65) - 65
      const fire = property * (COMMERCIAL_OCCUPANCY[str(values, 'occupancy')] ?? 0.002) * (COMMERCIAL_CONSTRUCTION[cls] ?? 1)
      const lines: PremiumLine[] = [{ label: 'Fire & allied perils', amount: fire }]
      if (flag(values, 'business_interruption')) lines.push({ label: 'Business interruption', amount: fire * 0.2, note: '20% loading' })
  ...
  home: {
    ...
    rate(values) {
      ...
      lines.push({ label: 'Typhoon & flood', amount: (building + contents) * (flag(values, 'flood_zone') ? 0.0025 : 0.0012) })
      return buildBreakdown(lines, NON_LIFE_TAXES, issues)
    }
  }
}
...
export function productFromLabel(label: string | null | undefined): ProductId | null {
  ...
  if (text.includes('comprehensive') || text === 'car' || text.includes('motor') || text.includes('auto')) return 'motor'
  if (text.includes('device') || text.includes('cellphone') || text.includes('phone') || text.includes('mobile')) return 'mobile'
